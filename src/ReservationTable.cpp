 ...
 * An index is required for each output entry, to avoid that multiple
 * invokations with different inputs returns the same output in the same clock
 * cycle. */
std::vector<int> ReservationTable::getReservations(const int port_in,
                                                   const int vc)
{
                                       ...
                                       const int port_out)
{
  ...
  for (int o = 0; o < n_outputs; o++)
  {
    for (vector<TReservation>::size_type i = 0;
         i < rtable[o].reservations.size(); i++)
    {
...
void ReservationTable::print()
{
  for (int o = 0; o < n_outputs; o++)
  {
    ...
    for (vector<TReservation>::size_type i = 0;
         i < rtable[o].reservations.size(); i++)
    {
      LOG << "<" << rtable[o].reservations[i].input << ","
...
void ReservationTable::release(const TReservation r, const int port_out)
{
       ...
       i != rtable[port_out].reservations.end(); i++)
  {
    if (*i == r)
    {
      ...
      vector<TReservation>::size_type removed_index =
          i - rtable[port_out].reservations.begin();
                                       ...
                                       const std::vector<int> &outputs)
{
  ...
  else
  {
    ...
    std::set<int> existing_ports_set(existing_ports.begin(),
                                     existing_ports.end());
...
// 重载方法：原子性地预留多个输出端口
void ReservationTable::reserve(const TReservation &r,
                               const vector<int> &outputs)
{
  assert(checkReservation(r, outputs) == RT_AVAILABLE); // 断言检查

  LOG << "[RT::reserve] input=" << r.input << " vc=" << r.vc << " outputs={";
  ...
  for (vector<int>::const_iterator it = outputs.begin(); it != outputs.end();
       ++it)
  {
    int port_out = *it;
    // 直接预留，避免重复的断言检查
...
// 重载方法：释放多个输出端口的预留
void ReservationTable::release(const TReservation &r,
                               const vector<int> &outputs)
{
  LOG << "[RT::release] input=" << r.input << " vc=" << r.vc << " outputs={";
  for (size_t i = 0; i < outputs.size(); ++i)
  {
    if (i > 0)
      LOG << ",";
