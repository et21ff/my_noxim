...
// DataDelta 转换器
template <> struct convert<DataDelta> {
...
// Trigger 转换器
template <> struct convert<Trigger> {
...
// RoleProperties 转换器
template <> struct convert<RoleProperties> {
...
template <> struct convert<CommandDefinition> {
...
// WorkspaceData 转换器
template <> struct convert<WorkspaceData> {
...
template <> struct convert<AtomicDispatchAction> {
...
template <> struct convert<DeltaEvent> {
...
template <> struct convert<ScheduleTemplate> {
...
// RoleWorkingSet 转换器
template <> struct convert<RoleWorkingSet> {
...
// DataFlowSpec 转换器
template <> struct convert<DataFlowSpec> {
  static bool decode(const Node &node, DataFlowSpec &rhs) {
    ...
    if (node["schedule_template"]) {
      rhs.schedule_template =
          std::unique_ptr<ScheduleTemplate>(new ScheduleTemplate());
    ...
    if (node["command_definitions"]) {
      rhs.command_definitions =
          node["command_definitions"].as<std::vector<CommandDefinition>>();
    } else {
...
// WorkloadConfig 转换器
template <> struct convert<WorkloadConfig> {
  static bool decode(const Node &node, WorkloadConfig &rhs) {
    ...
    if (node["data_flow_specs"]) {
      rhs.data_flow_specs =
          node["data_flow_specs"].as<std::vector<DataFlowSpec>>();
    } else {
  ...
  try {
    ...
    if (!root["workload"]) {
      throw std::runtime_error(
          "YAML file does not contain 'workload' root node");
    }

    return root["workload"].as<WorkloadConfig>();
  } catch (const YAML::Exception &e) {
    throw std::runtime_error("YAML parsing error in file '" + filepath +
                             "': " + e.what());
...
loadWorkloadConfigFromString(const std::string &yaml_content) {
  try {
    YAML::Node root = YAML::Load(yaml_content);

    if (!root["workload"]) {
      throw std::runtime_error(
          "YAML content does not contain 'workload' root node");
    }
    ...
    throw std::runtime_error(std::string("Error parsing YAML content: ") +
                             e.what());
  }
}
...
inline bool validateWorkloadConfig(const WorkloadConfig &config) {
  ...
  if (config.data_flow_specs.empty()) {
    std::cerr << "Validation Error: No data flow specifications found"
              << std::endl;
    return false;
  }
  ...
  for (const auto &spec : config.data_flow_specs) {
    if (spec.role.empty()) {
      std::cerr << "Validation Error: Data flow spec has empty role"
                << std::endl;
      return false;
    }
    ...
    if (spec.has_schedule()) {
      if (spec.schedule_template->total_timesteps <= 0) {
        std::cerr << "Validation Error: Invalid total_timesteps in role '"
                  << spec.role << "'" << std::endl;
        return false;
      }

      if (spec.schedule_template->delta_events.empty()) {
        std::cerr << "Validation Error: No delta events found in role '"
                  << spec.role << "'" << std::endl;
        return false;
      }
    }
    ...
    if (spec.has_commands()) {
      for (const auto &cmd : spec.command_definitions) {
        if (!cmd.is_valid()) {
          std::cerr << "Validation Error: Invalid command definition in role '"
                    << spec.role << "'" << std::endl;
          return false;
        }
      }
    ...
    if (!spec.has_schedule() && !spec.has_commands()) {
      std::cerr << "Validation Error: Role '" << spec.role
                << "' has neither schedule_template nor command_definitions"
                << std::endl;
      return false;
    }
  }
...
inline void printWorkloadConfig(const WorkloadConfig &config) {
    ...
    for (const auto &ws : config.working_set) {
      ...
      for (const auto &data : ws.data) {
        std::cout << "    - " << data.data_space << ": " << data.size
                  << " bytes (strategy: " << data.reuse_strategy << ")"
                  << std::endl;
      }
    }
  }
  ...
  for (const auto &spec : config.data_flow_specs) {
    ...
    if (spec.properties.compute_latency > 0) {
      ...
      std::cout << "      Compute Latency: " << spec.properties.compute_latency
                << std::endl;
    }
    ...
    if (spec.has_schedule()) {
      ...
      std::cout << "      Total Timesteps: "
                << spec.schedule_template->total_timesteps << std::endl;
    ...
    if (spec.has_commands()) {
      ...
      for (const auto &cmd : spec.command_definitions) {
        std::cout << "        - Command " << cmd.command_id << ": " << cmd.name
                  << std::endl;
        std::cout << "          Evict Payload: Weights="
                  << cmd.evict_payload.weights
