...
struct DataDelta {
  ...
  bool is_empty() const { return weights == 0 && inputs == 0 && outputs == 0; }

  size_t total_size() const { return weights + inputs + outputs; }
};
...
struct CommandDefinition {
  ...
  CommandDefinition(int id, const std::string &cmd_name,
                    const DataDelta &payload)
      : command_id(id), name(cmd_name), evict_payload(payload) {}

  bool is_valid() const { return command_id >= 0 && !name.empty(); }
};
...
struct ScheduleTemplate {
  ...
  bool is_valid() const { return total_timesteps > 0 && !delta_events.empty(); }
};
...
struct DataFlowSpec {
  ...
  DataFlowSpec(const DataFlowSpec &other)
      : role(other.role), properties(other.properties),
        command_definitions(other.command_definitions) {
    if (other.schedule_template) {
      schedule_template = std::unique_ptr<ScheduleTemplate>(
          new ScheduleTemplate(*other.schedule_template));
    }
  }
  ...
  bool has_commands() const { return !command_definitions.empty(); }
...
struct WorkloadConfig {
  ...
  find_properties_for_role(const std::string &role) const {
  ...
  find_commands_for_role(const std::string &role) const {
    const DataFlowSpec *spec = find_spec_for_role(role);
