...
vector<int> Router::route(const RouteData &route_data)
{
    ...
    if (GlobalParams::topology == TOPOLOGY_HIERARCHICAL)
    {
        ...
        else
        {
            ...
            if (route_data.command == -1)
            {
                ...
                assert(up_port_index != -1 && "No UP port found in hierarchical router for return packet");
                if (local_level > 0)
                {
            ...
            // 子情况2b: 使用预定义路由转发
            else if (use_predefined_routing && routing_patterns.count(route_data.data_type) > 0)
            {
    ...
    assert(!output_ports.empty() && "No output ports determined in hierarchical routing");
