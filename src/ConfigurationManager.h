...
namespace YAML
{
  ...
  struct convert<HubConfig>
  {
    ...
    {
      hubConfig.attachedNodes = node["attached_nodes"].as<vector<int>>(
          GlobalParams::default_hub_configuration.attachedNodes);
      hubConfig.rxChannels = node["rx_radio_channels"].as<vector<int>>(
          GlobalParams::default_hub_configuration.rxChannels);
      hubConfig.txChannels = node["tx_radio_channels"].as<vector<int>>(
          GlobalParams::default_hub_configuration.txChannels);
      hubConfig.toTileBufferSize = node["to_tile_buffer_size"].as<int>(
          GlobalParams::default_hub_configuration.toTileBufferSize);
      hubConfig.fromTileBufferSize = node["from_tile_buffer_size"].as<int>(
          GlobalParams::default_hub_configuration.fromTileBufferSize);
      hubConfig.txBufferSize = node["tx_buffer_size"].as<int>(
          GlobalParams::default_hub_configuration.txBufferSize);
      hubConfig.rxBufferSize = node["rx_buffer_size"].as<int>(
  ...
  struct convert<ChannelConfig>
  {
    ...
    static bool decode(const Node &node, ChannelConfig &channelConfig)
    {
      ...
      channelConfig.dataRate = node["data_rate"].as<int>(
  ...
  struct convert<RouterPowerConfig>
  {
    static bool decode(const Node &node, RouterPowerConfig &routerPowerConfig)
    {
      ...
      if (node["asymmetric_crossbar"])
      {
        cout << "[DEBUG] Found asymmetric_crossbar section, size="
             << node["asymmetric_crossbar"].size() << endl;
        for (YAML::const_iterator it = node["asymmetric_crossbar"].begin();
             it != node["asymmetric_crossbar"].end(); ++it)
        {
          vector<double> v = it->as<vector<double>>();
          array<double, 6> entry;
          copy(v.begin(), v.begin() + 6, entry.begin());
          routerPowerConfig.asymmetric_crossbar.push_back(entry);
        }
      }
