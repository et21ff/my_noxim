...
struct Flit
{
    ...
    {
        return (flit.src_id == src_id && flit.is_multicast == is_multicast && flit.dst_ids == dst_ids && flit.flit_type == flit_type && flit.vc_id == vc_id && flit.sequence_no == sequence_no && flit.sequence_length == sequence_length && flit.payload == payload && flit.logical_timestamp == logical_timestamp && flit.hop_no == hop_no && flit.use_low_voltage_path == use_low_voltage_path);
    }
};
