...
void infer_capabilities_from_workload(const WorkloadConfig &workload)
{
  ...
  for (const auto &spec : workload.data_flow_specs)
  {
    ...
    // 严格的边界检查
    assert(next_role_int <= ROLE_BUFFER &&
           "源角色是层级中的最后一级，不能有目标！");
...
void loadConfiguration()
{
      ...
      for (const auto &node : level_configs)
      {
        ...
        if (node["routing_patterns"])
        {
               ...
               ++it)
          {
            ...
            pattern.forward_count =
                pattern_config["forward_count"]
                    ? pattern_config["forward_count"].as<int>()
                    : 1;
            ...
            current_level_data.routing_patterns[data_type] = pattern;
          }
        }
  ...
  GlobalParams::traffic_distribution =
      readParam<string>(config, "traffic_distribution");
  GlobalParams::traffic_table_filename =
      readParam<string>(config, "traffic_table_filename");
  ...
  GlobalParams::n_virtual_channels =
      readParam<int>(config, "n_virtual_channels");
  GlobalParams::reset_time = readParam<int>(config, "reset_time");
  GlobalParams::stats_warm_up_time =
      readParam<int>(config, "stats_warm_up_time");
  ...
  GlobalParams::max_volume_to_be_drained =
      readParam<unsigned int>(config, "max_volume_to_be_drained");
  // GlobalParams::hotspots;
  GlobalParams::show_buffer_stats =
      readParam<bool>(config, "show_buffer_stats");
       ...
       hubs_it != config["Hubs"].end(); ++hubs_it)
  {
    ...
    copy(GlobalParams::hub_configuration[hub_id].rxChannels.begin(),
         GlobalParams::hub_configuration[hub_id].rxChannels.end(),
         inserter(channelSet, channelSet.end()));
    copy(GlobalParams::hub_configuration[hub_id].txChannels.begin(),
         GlobalParams::hub_configuration[hub_id].txChannels.end(),
         inserter(channelSet, channelSet.end()));
  }
...
void showHelp(char selfname[])
{
  cout
      ...
      << "\t-winoc_dst_hops\t\t\tMax number of hops between target RadioHub "
      ...
      << "\t\tTABLE_BASED FILENAME\tRouting Table Based routing algorithm with "
      ...
      << endl
      << "\t\t\t\tbeen delivered" << endl
      << "\t-asciimonitor\t\tShow status of the network while running "
         "(experimental)"
      << endl
...
void configure(int arg_num, char *arg_vet[])
{
  ...
  loadConfiguration();
  GlobalParams::workload =
      loadWorkloadConfigFromFile(GlobalParams::config_filename);
