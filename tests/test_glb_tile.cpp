        ...
        else
        {
            if (req_rx.read() == 1 - current_level_rx)
            {
                ...
                cout << "[" << sc_time_stamp() << "] " << node_name
                     ...
                     //  << " dst=" << received_flit.dst_ids
                     << " vc=" << received_flit.vc_id
                     << " type=" << received_flit.flit_type
                     << " data_type=" << DataType_to_str(received_flit.data_type)
                     << " seq_no=" << received_flit.sequence_no
                     << " cmd=" << received_flit.command
                     << " current_forward=" << received_flit.current_forward
                     << " forward_count=" << received_flit.forward_count << endl;

                if (received_flit.flit_type == FLIT_TYPE_TAIL)
                {
                    cout << "[" << sc_time_stamp() << "] " << node_name
                         << " completed receiving packet from src=" << received_flit.src_id
                         << " vc=" << received_flit.vc_id
                         << " data_type=" << DataType_to_str(received_flit.data_type)
                         << " total flits received=" << received_flit.sequence_length
                         << " timestamp=" << received_flit.logical_timestamp << endl;
                }
